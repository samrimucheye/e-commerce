...
export default function ProductCard({ product }: ProductCardProps) {
    ...
    return (
        <motion.div
            ...
            className="group relative bg-white dark:bg-slate-900 rounded-[2.5rem] overflow-hidden border border-slate-100 dark:border-slate-800 shadow-xl hover:shadow-2xl transition-all duration-500"
        >
            {/* Badge Overlay */}
            <div className="absolute top-5 left-5 z-20 flex flex-col gap-2" style={{ transform: "translateZ(50px)" }}>
                {product.isNewArrival && (
                    <div className="bg-emerald-500 text-white text-[10px] font-black px-3 py-1.5 rounded-xl flex items-center shadow-lg shadow-emerald-500/20 backdrop-blur-md">
                        <Sparkles className="w-3 h-3 mr-1" /> NEW
                    </div>
                )}
                {product.isOnSale && (
                    <div className="bg-rose-500 text-white text-[10px] font-black px-3 py-1.5 rounded-xl flex items-center shadow-lg shadow-rose-500/20 backdrop-blur-md">
            ...
            <Link href={`/products/${product.slug}`} className="block relative aspect-[4/5] overflow-hidden">
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-slate-950/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex items-end p-8">
                    <div className="flex w-full gap-2 translate-y-4 group-hover:translate-y-0 transition-transform duration-500">
                        <button className="flex-1 bg-white text-slate-950 py-3 rounded-2xl font-bold text-sm flex items-center justify-center hover:bg-indigo-50 transition-colors">
            ...
            <div className="p-8 space-y-4" style={{ transform: "translateZ(30px)" }}>
                <div className="flex items-center gap-1">
                    {[...Array(5)].map((_, i) => (
                        <Star key={i} className={`w-3.5 h-3.5 ${i < 4 ? 'text-amber-400 fill-amber-400' : 'text-slate-200 dark:text-slate-700'}`} />
                    ))}
                    <span className="text-[10px] font-bold text-slate-400 ml-1">(124 reviews)</span>
                </div>
                ...
                <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-2">
                        {product.isOnSale && product.salePrice ? (
                            <>
                                <span className="text-2xl font-black text-slate-950 dark:text-white">${product.salePrice}</span>
